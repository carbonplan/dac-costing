...
class CostWidget(object):
    ...
    def app(self):
        ...
        rsliders['Scale [tCO2/year]'] = FloatLogSlider(min=1, max=12, step=.1, value=app_params['Scale [tCO2/year]'])
        rsliders['DAC Capacity Factor'] = FloatSlider(min=0, max=1, step=0.01, readout_format='.2%', value=app_params['DAC Capacity Factor'])
        rsliders['DAC Section Lead Time [years]'] = IntSlider(min=1, max=6, value=app_params['DAC Section Lead Time [years]'])
        rsliders['Total Capex [$]'] = FloatSlider(value=app_params['Total Capex [$]'])
        rsliders['Electric Power Requierement [MW]'] = FloatSlider(value=app_params['Electric Power Requierement [MW]'])
        rsliders['Thermal [GJ/tCO2]'] = FloatSlider(value=app_params['Thermal [GJ/tCO2]'])
        rsliders['Fixed O+M Costs [$/tCO2]'] = FloatSlider(value=app_params['Fixed O+M Costs [$/tCO2]'])
        rsliders['Varible O+M Cost [$/tCO2]'] = FloatSlider(value=app_params['Varible O+M Cost [$/tCO2]'])
        ...
        # economic parameters
        esliders['Economic Lifetime [years]'] = IntSlider(min=1, max=50, value=app_params['Economic Lifetime [years]'])
        esliders['WACC [%]'] = FloatSlider(min=0, max=1, step=0.01, readout_format='.2%', value=app_params['WACC [%]'])
        esliders['Natural Gas Cost [$/mmBTU]'] = FloatSlider(min=0, max=10, step=0.1, value=app_params['Natural Gas Cost [$/mmBTU]'])
